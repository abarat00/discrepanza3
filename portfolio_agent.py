...
class PortfolioAgent:
    ...
    def train(
        ...
        checkpoint_freq=10,
        resume_from=None,
        early_stop_patience=75   # Numero di episodi senza miglioramento per early stopping
    ):
        ...
        for episode in range_total_episodes:
            ...
            while not done:
                ...
                if done:
                    ...
                    current_portfolio_value = np.mean(portfolio_values)#
                    print(f"Episodio {episode}: Ricompensa media = {current_mean_reward:.4f}, Valore portafoglio medio = {current_portfolio_value:.4f}, Best Reward = {best_mean_reward:.4f}, Best Portafoglio = {best_portfolio_value:.4f}, No Improvement Count = {no_improvement_count}")#
                    ...
                    if (current_mean_reward > best_mean_reward) or (current_portfolio_value > best_portfolio_value) :
                        #best_mean_reward = current_mean_reward
                        best_mean_reward = max(best_mean_reward, current_mean_reward)#
                        best_portfolio_value = max(best_portfolio_value, current_portfolio_value)#
                        no_improvement_count = 0
                    else:
            ...
            if early_stop:
                print("Interruzione dell'addestramento per early stopping.")
                break
