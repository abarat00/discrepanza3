...
# Aggiungi questo alla classe PortfolioEnvironment:
    ...
    def calculate_trading_costs(self, actions, current_prices):
        ...
        # Aggiorna il contatore totale delle commissioni
        self.total_commission_paid += total_cost
        self.commission_history.append(total_cost)
    ...
    def step(self, actions):
        ...
        self.commission_history.append(trading_costs)
        portfolio_value_prev = self.portfolio_values_history[-1]
        portfolio_value = self.get_portfolio_value()
        self.portfolio_values_history.append(portfolio_value)
        self.cash_history.append(self.cash)
        #if self.writer is not None:
            #self.writer.add_scalar("Portfolio/CommissionCost", trading_costs, self.current_index)
            #self.writer.add_scalar("Portfolio/TotalCommissionPaid", self.total_commission_paid, self.current_index)
    ...
    def get_real_portfolio_metrics(self, price_feature="adjClose"):
        ...
        commission_ratio = (total_commission / final_value) * 100 if final_value > 0 else 0
        trades_with_commission = sum(1 for cost in self.commission_history if cost > 0)
        avg_commission = total_commission / max(1, trades_with_commission)
        ...
        return {
            ...
            'commission_to_portfolio_ratio': commission_ratio,
            'avg_commission_per_trade': avg_commission,
            'trades_with_commission': trades_with_commission
