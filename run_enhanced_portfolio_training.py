...
def plot_enhanced_performance(results, output_dir, tickers, enhancement_config, test_data=None):
    ...
    if 'cumulative_rewards' in test_data and len(test_data['cumulative_rewards']) > 0:
        # Usa i dati calcolati dalle reward di test
        ax1.plot(test_data['cumulative_rewards'], linewidth=2, color='darkgreen')
        ax1.set_title('Test Cumulative Reward')
        ax1.set_xlabel('Step')
        ax1.set_ylabel('Cumulative Reward')
        ax1.grid(True, alpha=0.3)
    elif 'cum_rewards' in results and len(results['cum_rewards']) > 0:
        # Fallback ai dati di training se disponibili
        safe_plot(ax1, results['cum_rewards'], 'Cumulative Reward (Training)', 'Episodes (x5)', 'Mean Reward')
    else:
    ...
    if test_data.get('portfolio_value_log') is not None and len(test_data['portfolio_value_log']) > 0:
        ...
        # Correggi l'ultimo valore se mostra un calo anomalo
        if len(portfolio_values) > 2 and portfolio_values[-1] < portfolio_values[-2] * 0.9:
            # Se l'ultimo valore è inferiore al 90% del penultimo, usa il penultimo
            portfolio_values_corrected = portfolio_values.copy()
            portfolio_values_corrected[-1] = portfolio_values[-2]
        else:
            portfolio_values_corrected = portfolio_values
        
        # Usa la versione corretta per il grafico
        ax2.plot(portfolio_values_corrected, linewidth=2, color='darkblue')
        ...
        if len(portfolio_values) > 1:
            # Usa l'ultimo valore corretto per il calcolo del rendimento
            final_value = portfolio_values_corrected[-1]
    ...
    if 'test_metrics' in test_data and 'sharpe_ratio' in test_data['test_metrics']:
        ...
        # Crea un grafico a barre migliore
        bar_width = 0.6
        ax3.bar(['Test Sharpe'], [test_sharpe], color='navy', width=bar_width)
        ax3.set_title('Test Sharpe Ratio', fontsize=14)
        ax3.set_ylabel('Sharpe Ratio', fontsize=12)
        
        # Migliora le linee di riferimento
        ax3.axhline(y=1.0, color='green', linestyle='--', alpha=0.7, linewidth=1.5)
        ax3.text(-0.2, 1.05, 'Good (1.0)', fontsize=9, color='green')
        
        ax3.axhline(y=0.5, color='orange', linestyle='--', alpha=0.7, linewidth=1.5)
        ax3.text(-0.2, 0.55, 'Moderate (0.5)', fontsize=9, color='orange')
        
        ax3.axhline(y=0.0, color='red', linestyle='--', alpha=0.7, linewidth=1.5)
        ax3.text(-0.2, 0.05, 'Poor (0.0)', fontsize=9, color='red')
        
        # Aggiungi il valore dello Sharpe sopra la barra
        ax3.text(0, test_sharpe + 0.05, f"{test_sharpe:.4f}", 
                ha='center', va='bottom', fontsize=11, fontweight='bold')
        
        # Imposta limiti dell'asse y in modo più intelligente
        max_y = max(1.5, test_sharpe * 1.2)  # Almeno fino a 1.5 o 20% oltre il valore 
        min_y = min(-0.2, test_sharpe - 0.2)
        ax3.set_ylim([min_y, max_y])
        
        # Imposta limiti dell'asse x per mostrare meglio il grafico
    ...
    profiles = {
        'active': {
            ...
            'train': {
                ...
                'encoding_size': 32,
                'clip_grad_norm': 1.0,
                'early_stop_patience':100  # Minore pazienza
            },
        ...
        'longterm': {
            ...
            'train': {
                ...
                'encoding_size': 32,
                'clip_grad_norm': 1.0,
                'early_stop_patience': 240  # Maggiore pazienza
            },
            # Parametri per environment
            'env': {
        ...
        'momentum': {
            ...
            'train': {
                ...
                'encoding_size': 32,
                'clip_grad_norm': 1.0,
                'early_stop_patience': 160  # Pazienza media
            },
            # Parametri per environment
            'env': {
...
def main(args):
    ...
    enhancement_config = {
        ...
        'position_stability_factor': profile_config['env']['position_stability_bonus_factor'],
    ...
    training_config.update({
        ...
        #'weight_decay_actor': profile_config['train']['weight_decay_actor'],  # Aggiungi questa riga
        #'weight_decay_critic': profile_config['train']['weight_decay_critic'],
        ...
        'early_stop_patience': profile_config['train']['early_stop_patience']
    })

    print(f"Early stop patience from profile: {profile_config['train']['early_stop_patience']}")
    print(f"Early stop patience in training config: {training_config['early_stop_patience']}")
    ...
    test_metrics_file = f'{output_dir}/test/test_metrics.json'
    rewards_log_file = f'{output_dir}/test/rewards_log.npy'
    ...
    if os.path.exists(rewards_log_file):
        rewards = np.load(rewards_log_file)
        # Calcola la reward cumulativa sommando progressivamente i valori
        cumulative_rewards = np.cumsum(rewards)
        test_data['cumulative_rewards'] = cumulative_rewards
    else:
        print(f"Avviso: File {rewards_log_file} non trovato. Non sarà possibile visualizzare le ricompense cumulative.")
        #test_data['cumulative_rewards'] = []
        ...
        if last_model:
            ...
            if args.use_backtest_framework:
                ...
                while not done:
                    ...
                    if step_counter < len(next(iter(aligned_dfs_test.values()))) - 1:
                        portfolio_value_log.append(test_env.get_portfolio_value())
                    else:
                        if portfolio_value_log:
                            portfolio_value_log.append(portfolio_value_log[-1])
                        else:
                            portfolio_value_log.append(test_env.get_portfolio_value())
                ...
                print("\n===== ANALISI COMMISSIONI =====")
                print(f"Commissioni totali pagate: ${metrics['total_commission_paid']:.2f}")
                print(f"Commissioni come % del portafoglio finale: {metrics['commission_to_portfolio_ratio']:.2f}%")
                print(f"Commissione media per operazione: ${metrics['avg_commission_per_trade']:.2f}")
                print(f"Numero di operazioni con commissioni: {metrics['trades_with_commission']}")
